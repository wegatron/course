...
namespace pcl
{
	namespace registration
	{
		...
		TransformationEstimationLM_adv<PointSource, PointTarget, MatScalar>::TransformationEstimationLM_adv()
			: tmp_src_()
			, tmp_tgt_()
			, tmp_idx_src_()
			, tmp_idx_tgt_()
		...
		template <typename PointSource, typename PointTarget, typename MatScalar> void
			TransformationEstimationLM_adv<PointSource, PointTarget, MatScalar>::estimateRigidTransformation(
				...
				Matrix4 &transformation_matrix) const
		{
		  // <cloud_src,cloud_src> is the source dataset
			...
			{
				PCL_ERROR("[TransformationEstimationLM_adv::estimateRigidTransformation] ");
			...
			PCL_DEBUG("[TransformationEstimationLM_adv::estimateRigidTransformation]");
				...
				Matrix4 &transformation_matrix) const
		{
			...
			{
				PCL_ERROR("[TransformationEstimationLM_adv::estimateRigidTransformation] Number or points in source (%lu) differs than target (%lu)!\n", indices_src.size(), cloud_tgt.points.size());
				return;
			}
				...
				Matrix4 &transformation_matrix) const
		{
			...
			//OptimizationFunctorWithIndices_adv functor(static_cast<int> (indices_src.size()), this);
			...
			//Eigen::LevenbergMarquardt<OptimizationFunctorWithIndices_adv, MatScalar> lm(functor);
			int info = lm.minimize(x);

			// Compute the norm of the residuals
			PCL_DEBUG("[TransformationEstimationLM_adv::estimateRigidTransformation] LM solver finished with exit code %i, having a residual norm of %g. \n", info, lm.fvec.norm());
			PCL_DEBUG("Final solution: [%f", x[0]);
			for (int i = 1; i < n_unknowns; ++i)
				PCL_DEBUG(" %f", x[i]);
			PCL_DEBUG("]\n");
		...
		template <typename PointSource, typename PointTarget, typename MatScalar> int
			TransformationEstimationLM_adv<PointSource, PointTarget, MatScalar>::OptimizationFunctor::operator () (
		...
		//////////////////////////////////////////////////////////////////////////////////////////////
		template <typename PointSource, typename PointTarget, typename MatScalar> int
			TransformationEstimationLM_adv<PointSource, PointTarget, MatScalar>::OptimizationFunctorWithIndices::operator() (
				const VectorX &x, VectorX &fvec) const
		{
			const PointCloud<PointSource> & src_points = *estimator_->tmp_src_;
			const PointCloud<PointTarget> & tgt_points = *estimator_->tmp_tgt_;
        ...
        template<typename PointSource, typename PointTarget, typename MatScalar>
        int
        TransformationEstimationLM_adv<PointSource, PointTarget, MatScalar>::OptimizationFunctorWithIndices_adv::operator()(
                const TransformationEstimationLM_adv::VectorX &x, TransformationEstimationLM_adv::VectorX &fvec) {
            // get target
            auto idx_src = estimator_->tmp_idx_src_;
            auto src = estimator_->tmp_src_;
            auto idx_tgt = estimator_->tmp_idx_tgt_;
            auto tgt = estimator_->tmp_tgt_;

            estimator_->wrap_point_->setParam(x);
            for(int i=0; i<m_data_points; ++i)
            {
                Vector4 src_warped = estimator_->warp_point_->warpPoint(src[idx_src[i]]);
                fvec[i] =  (src_warped.block<3,1>(0,0)
                        - Eigen::Matrix<MatScalar, 3, 1>(tgt[idx_tgt[i]].x,
                                tgt[idx_tgt[i]].y, tgt[idx_tgt[i]].z)).norm();
            }
            return 0;
        }

        template<typename PointSource, typename PointTarget, typename MatScalar>
        int TransformationEstimationLM_adv<PointSource, PointTarget, MatScalar>::OptimizationFunctorWithIndices_adv::df(
                const TransformationEstimationLM_adv::VectorX &x,
                Eigen::Matrix<MatScalar, Eigen::Dynamic, Eigen::Dynamic> &jac) const {
            // TODO ZSW
            Eigen::Quaternion<MatScalar> q;
            q.toRotationMatrix();
            return 0;
        }
    }
