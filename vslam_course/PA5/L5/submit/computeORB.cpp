...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        if(kp.pt.x<8 || kp.pt.x>image.cols-8 || kp.pt.y<8 || kp.pt.y>image.cols-8) continue;
        float m[2] = {0, 0};
        for(int y=-8; y<8; ++y) {
            for(int x=-8; x<8; ++x)
            {
                m[0] += x * image.at<uchar>(kp.pt.y+y, kp.pt.x+x);
                m[1] += y * image.at<uchar>(kp.pt.y+y, kp.pt.x+x);
            }
        }
        kp.angle = 180 / pi * std::atan2(m[1], m[0]);
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        float u = kp.pt.x;
        float v = kp.pt.y;
        int * pattern_ptr = ORB_pattern;
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            d[i] = 0;  // if kp goes outside, set d.clear()
            float cos_t = cos(kp.angle*pi/180);
            float sin_t = sin(kp.angle*pi/180);
            float pu = u + cos_t * pattern_ptr[0] - sin_t * pattern_ptr[1];
            float pv = v + sin_t * pattern_ptr[0] + cos_t * pattern_ptr[1];
            float qu = u + cos_t * pattern_ptr[2] - sin_t * pattern_ptr[3];
            float qv = v + sin_t * pattern_ptr[2] + cos_t * pattern_ptr[3];
            pattern_ptr += 4;
            if(pu<0 || pv<0 || qu<0 || qv<0
               || pu>=image.cols || qu>=image.cols
               || pv>=image.rows || qv>=image.rows) {
                d.clear();
                break;
            }
            d[i] = image.at<uchar>(pv, pu) > image.at<uchar>(qv, qu);
	    // END YOUR CODE HERE
        }
        desc.push_back(d);
    }
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for(int i=0; i<desc1.size(); ++i)
    {
        ...
        for(int j=0; j<desc2.size(); ++j)
        {
            ...
            {
                dis += (desc1[i][ind] != desc2[j][ind]);
            }
            if(dis < min_dis) { min_dis = dis; min_dis_index = j; }
        }
        if(min_dis_index != -1)
        {
            matches.emplace_back(i, min_dis_index, min_dis);
        }
    }
    // END YOUR CODE HERE
    auto tp1 = std::chrono::high_resolution_clock::now();
    std::cout << "match time cost: "
