...
namespace gr
{
	namespace Demo
	{
		...
		static int max_time_seconds = 1000;
		                          ...
		                          const Utils::Logger& logger = Utils::Logger())
		{
			...
			while (i < argc)
			{
				...
				{
					set_cur_pos(Eigen::Vector3f(atof(argv[++i]), atof(argv[++i]), atof(argv[++i])));
				}
				else if (!strcmp(argv[i], "--id"))
				{
					set_cur_id(atoi(argv[++i]));
				}
				else if (!strcmp(argv[i], "-gt_file"))
				{
					gt_file = argv[++i];
				}
			...
			// set ground truth
			if (!gt_file.empty()) {
				std::ifstream gifs(gt_file, std::ifstream::binary);
				gifs.seekg(cur_id() * 12 * sizeof(double), gifs.beg);
				double data[12];
				gifs.read(reinterpret_cast<char*>(data), sizeof(double) * 12);
				Eigen::Affine3f rt;
				rt.matrix() << data[0], data[1], data[2], data[9],
					data[3], data[4], data[5], data[10],
