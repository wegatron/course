...
void MATCH_BASE_TYPE::init(const std::vector<Point3D>& P,
    ...
    if (Q.size() > options_.sample_size){
		...
		std::cout << "Q uniform pt size:" << uniform_Q.size() << std::endl;
		std::cout << "option Q size:" << options_.sample_size << std::endl;

		size_t sample_size = max(options_.sample_size, uniform_Q.size() / 2);
		std::cout << "used Q size: " << sample_size << std::endl;

        size_t nbSamples = std::min(uniform_Q.size(), sample_size);
	...
	set_sample_p(&sampled_P_3D_);
	set_sample_q(&sampled_Q_3D_);
