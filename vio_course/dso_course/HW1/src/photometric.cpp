...
hw::PhotometricUndistorter::PhotometricUndistorter(std::string &path)
{
    ...
    // cv::waitKey(0);
    //std::cout <<  vignette_inv_ << std::endl;
    // std::cout << vignette_ << std::endl;
}
...
void hw::PhotometricUndistorter::processFrame(cv::Mat& image)
{
    ...
    for(int i=0; i<h_img; ++i) {
      for(int j=0; j<w_img; ++j) {
        uint8_t ori_val = image.at<uint8_t>(i,j);
        int nv = G_[ori_val]*vignette_inv_.at<float>(i,j);
        image.at<uint8_t>(i,j) = nv > 255 ? 255 : nv;
      }
    }
}
