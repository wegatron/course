...
//bool epnp(const std::vector<cv::Point3f> &obj_pts,
//          const std::vector<cv::Point2f> &img_pts,
//          const Eigen::Matrix3d &K,
//          const int iter_times,
//          Eigen::Matrix4d &T_wc)
//{
//    const size_t n = obj_pts.size();
//    std::vector<size_t> inds(n);
//    for(size_t i=0; i<n; ++i) inds[i] = i;
//    std::default_random_engine generator;
//    cv::Point3f select_obj_pts[4];
//    cv::Point2f select_img_pts[4];
//    cv::Mat r, t, refined_rt;
//    std::vector<bool> inliers(n, false);

//    for(int i=0; i<iter_times; ++i)
//    {
//        // select 4 ransac point
//        for(size_t j=0; j<4; ++j)
//        {
//            std::uniform_int_distribution<int> distribution(0, n - j);
//            int id = distribution(generator);
//            size_t index = inds[id];
//            select_obj_pts[j] = obj_pts[index];
//            select_img_pts[j] = img_pts[index];
//            std::swap(inds[id], inds[n-j-1]);
//        }

//        // compute pose
//        compute_pose(cv_K, select_obj_pts, select_img_pts, r, t);

//        // check inliers
//        int in_num = check_inliers(K, r, t, obj_pts, img_pts, inliers);
//        if(in_num < 10) continue;

//        // refine and check inliers again
//        if(refine())
//        {
//            T_wc = refined_rt;
//            return true;
