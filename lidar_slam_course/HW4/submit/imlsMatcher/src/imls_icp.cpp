                                         ...
                                         double& height)
{
    ...
    for(int i=0; i<nearPoints.size(); ++i) {
        auto diff = x - nearPoints[i];
        double weight = std::exp(-diff.squaredNorm()/(m_h * m_h));
        tmp_val += weight * diff.transpose() * nearNormals[i];
        sum_weight += weight;
    }
    height = tmp_val / sum_weight;
    //end of TODO
    return true;
}
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    A -= 1.0/nearPoints.size() * center * center.transpose();
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix2d> eigen_solver(A);
    ...
    auto evs = eigen_solver.eigenvectors();
    normal = evs.col(0);
    //end of TODO
