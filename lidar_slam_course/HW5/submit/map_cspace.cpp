	     ...
	     unsigned char* marked)
{
  ...
  //这里的距离是栅格的距离
  unsigned int di = abs(static_cast<long long>(i) - static_cast<long long>(src_i));
  unsigned int dj = abs(static_cast<long long>(j) - static_cast<long long>(src_j));
  ...
  for(int i=0; i<map->size_x; i++)
  {
      ...
      for(int j=0; j<map->size_y; j++)
      {
          if(map->cells[MAP_INDEX(map, i, j)].occ_state == CELL_STATUS_OCC)
          {
              ...
              map->cells[MAP_INDEX(map, i, j)].score = 0;
