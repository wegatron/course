...
public:
    ...
    */
    ...
    void Lidar_MotionCalibration(
            ...
            int& beam_number)
    {
        auto rotation_start = frame_start_pose.getRotation();
        auto rotation_end = frame_end_pose.getRotation();

        auto translate_start = frame_start_pose.getOrigin();
        auto translate_end = frame_end_pose.getOrigin();

        auto world2base = frame_base_pose.inverse();
        // index range [startIndex+1, startIndex+beam_number-1]
        for(int ind=1; ind<beam_number; ++ind)
        {
            // adjust startIndex+ind+1
            // linear interpolation
            auto cur_translate = translate_start.lerp(translate_end, ind/(beam_number-1));
            auto cur_rotation = rotation_start.slerp(rotation_end, ind/(beam_number-1));
            tf::Pose cur2world(cur_rotation, cur_translate);

            // cur 2 base
            auto cur2base = world2base * cur2world;
            tf::Vector3 pt(ranges[startIndex+ind+1] * cos(angles[startIndex+ind+1]), ranges[startIndex+ind+1] * sin(angles[startIndex+ind+1]), 0.0);
            auto deskew_pt = cur2base*pt;
            ranges[startIndex+ind+1] = sqrt(deskew_pt.x() * deskew_pt.x() + deskew_pt.y() * deskew_pt.y());
            //ranges[startIndex+ind+1] = 0;
            angles[startIndex+ind+1] = atan2(deskew_pt.y(), deskew_pt.x());
        }
    }
