...
Eigen::Matrix3d pos2rt(const Eigen::Vector3d &pos)
{
    Eigen::Matrix3d rt;
    rt << cos(pos(2)), -sin(pos(2)), pos(0),
            sin(pos(2)), cos(pos(2)), pos(1),
            0,0, 1;
    return rt;
}

Eigen::Vector3d rt2pos(const Eigen::Matrix3d &rt)
{
    Eigen::Vector3d pos;
    pos << rt(0,2), rt(1,2), atan2(rt(1,0), rt(0,0));
    return pos;
}
...
Eigen::Vector3d  cal_delta_distence(Eigen::Vector3d odom_pose)
{
    ...
    // now_pos - last_pos
    Eigen::Matrix3d now2world = pos2rt(now_pos);
    Eigen::Matrix3d last2world = pos2rt(last_pos);
    Eigen::Matrix3d now2last = last2world.inverse() * now2world;
    d_pos = rt2pos(now2last);
    return d_pos;
}
